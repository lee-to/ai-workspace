...
The `--name` flag overrides the name from `.json`, and `--group` is additive. Running `sync` also reconciles the database with `.ai-workspace.json` if present. Shared paths from config must exist and resolve inside the project directory. Event history is intentionally not exported.
